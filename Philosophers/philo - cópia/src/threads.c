...
/*   Updated: 2024/02/04 18:34:25 by pevieira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	checker_philo_life(t_philo *philo)
{
	...
	if (get_time() - philo->last_meal >= philo->table->death_time \
		&& !finish_simulation(philo, false))
	{
	...
	else if (philo->table->nb_meals > 0 && \
		philo->meal_count >= philo->table->nb_meals)
	{
